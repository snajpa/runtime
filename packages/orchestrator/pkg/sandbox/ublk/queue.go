...
func (o *queueOwner) handle(tag uint16) int32 {
	...
	// COMMIT_AND_FETCH couples completion to admission of the next tag. Once
	// abort is requested, complete newly visible work with an error instead of
	// entering the backend; the owner must still commit the tag so STOP can
	// quiesce the queue.
	if o.dev.abortRequested.Load() {
