...
func TestHandleOps(t *testing.T) {
	...
	t.Run("abort gates backend admission", func(t *testing.T) {
		t.Parallel()

		owner, backend := newTestQueue(ioOpDiscard, 8, 0)
		owner.dev.abortRequested.Store(true)

		if got, want := owner.handle(0), -int32(unix.EINTR); got != want {
			t.Errorf("result = %d, want %d", got, want)
		}
		if got := backend.zeroCount(); got != 0 {
			t.Errorf("abort entered backend %d times, want 0", got)
		}
	})
