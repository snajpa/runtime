...
func TestDeferredFaultsKeepsWPDistinctFromMissing(t *testing.T) {
	...
	for _, pf := range out {
		...
		missing = true
		require.Zero(t, pf.flags&UFFD_PAGEFAULT_FLAG_WRITE,
			"the WP fault's WRITE flag must not leak into the MISSING entry")
	}

	require.True(t, wp, "the WP fault must survive the dedup")
	require.True(t, missing, "the MISSING fault must survive the dedup")
