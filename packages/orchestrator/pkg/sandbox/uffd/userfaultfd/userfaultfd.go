...
// called OUTSIDE settleRequests: even the worst-case backoff (three retries,
// 50/100/200 ms plus jitter) must never block the REMOVE batch, which needs
// the write lock to record removed pages and to cancel a CoW window (S-28). On success the returned buffer is owned by the
